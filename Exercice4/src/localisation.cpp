...
namespace SimpleGE
{
  ...
  [[nodiscard]] std::string Localisation::GetImpl(std::string_view key, const LocaleContext& queryContext) const
  {
    ...
    std::string localisedSentence = localized->second;

    for (const std::pair<const std::string, std::string>& n : mergedContext)
    {
      std::string searchedKey = "{" +n.first + "}";
      int position = localisedSentence.find(searchedKey);
      while (position != std::string::npos)
      {
        localisedSentence.replace(position, searchedKey.length(), n.second);
        position = localisedSentence.find(searchedKey);
      }
    }
